...
class Home extends \app\core\Controller {
    ...
    public function index($request) {
        ...
        // get the phpunit.xml stub file location
        $app_dir = dirname(__DIR__);
        $tests = str_replace('\\', '/', explode('|', $request->data['test_files']));
        
        // determine if any file was selected
        if ( strlen($tests[0]) == 0 ) {
    ...
    <testsuite name="application">
        <directory suffix=".test.php">$test_directory</directory>
    </testsuite>

EOT;

        } else {
            // 1 or more files were selected
            $file = '';
            
            // loop through the selected files and run them
...
EOT;
        ...
        // create the phpunit.xml configuration files from stub file
        $stub = file_get_contents($app_dir . '\config\stub.php');
        file_put_contents($app_dir . '\config\phpunit.xml', str_replace("#replace#", $file, $stub));
